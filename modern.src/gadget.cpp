...
static bool gadget_is_outside_legacy_tactical(int x, int y, int w, int h)
{
...
static void convert_legacy_ui_mouse_for_gadget(int gadget_x, int gadget_y, int gadget_w, int gadget_h,
	int& mousex, int& mousey)
{
	if (!Render_Bridge_Use_Legacy_Sidebar_Mode()) {
		return;
	}
	if (!gadget_is_outside_legacy_tactical(gadget_x, gadget_y, gadget_w, gadget_h)) {
		return;
	}
...
{
#ifdef USE_RENDER_BRIDGE
	convert_legacy_ui_mouse_for_gadget(X, Y, Width, Height, mousex, mousey);
#endif

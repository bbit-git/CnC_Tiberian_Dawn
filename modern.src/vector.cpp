...
int DynamicVectorClass<T>::Add(T const & object)
{
	if (this->ActiveCount >= this->Length()) {
		if ((this->IsAllocated || !this->VectorMax) && this->GrowthStep > 0) {
			if (!this->Resize(this->Length() + this->GrowthStep)) {
...
int DynamicVectorClass<T>::Add_Head(T const & object)
{
	...
	*/
	if (this->ActiveCount) {
		memmove(&(*this)[1], &(*this)[0], this->ActiveCount * sizeof(T));
	}
	(*this)[0] = object;
	this->ActiveCount++;
...
int DynamicVectorClass<T>::Delete(T const & object)
{
	int index = ID(object);
	if (index != -1){
		return(DynamicVectorClass<T>::Delete((int)index));
	}else{
		return (false);
	}
}

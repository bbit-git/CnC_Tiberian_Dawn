...
void CellClass::Decode_Pointers(void)
{
	...
	if (Overlapper[0]) {
		Overlapper[0] = As_Object((TARGET)(intptr_t)Overlapper[0]);
		Check_Ptr((void *)Overlapper[0],__FILE__,__LINE__);
	}

	if (Overlapper[1]) {
		Overlapper[1] = As_Object((TARGET)(intptr_t)Overlapper[1]);
		Check_Ptr((void *)Overlapper[1],__FILE__,__LINE__);
	}

	if (Overlapper[2]) {
		Overlapper[2] = As_Object((TARGET)(intptr_t)Overlapper[2]);
	...
	*/
	if (IsTrigger) {
		CellTriggers[Cell_Number()] = As_Trigger( (TARGET)(intptr_t)CellTriggers[Cell_Number()] );

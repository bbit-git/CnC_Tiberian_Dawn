...
bool Do_Reinforcements(TeamTypeClass *teamtype)
{
	...
	*/
	...
	for (int idx=0; idx < teamtype->ClassCount; idx++) {
		if (teamtype->Class[idx]->IsTransporter || teamtype->Class[idx]->What_Am_I() == RTTI_AIRCRAFTTYPE) {
			hastransport = true;
			if (teamtype->Class[idx]->What_Am_I() == RTTI_AIRCRAFTTYPE) {
				airtransport = true;
			} else {
				watertransport = (((UnitTypeClass const *)teamtype->Class[idx])->Type == UNIT_HOVER);
			}
		} else {
			onlytransport = false;
		}
	...
	*/
	...
	for (int idx = 0; idx < teamtype->ClassCount; idx++) {
		TechnoTypeClass const * tclass = teamtype->Class[idx];

		for (int sub = 0; sub < teamtype->DesiredNum[idx]; sub++) {
			ScenarioInit++;
			...
			if (!another && (type->What_Am_I() == RTTI_UNITTYPE || type->What_Am_I() == RTTI_INFANTRYTYPE)) {
				...
				*/
				if (house->Edge == SOURCE_SOUTH) {
					bool found = false;
					for (int idx = Map.MapCellX; idx < Map.MapCellX+Map.MapCellWidth-1; idx++) {
						CELL cell = XY_Cell(idx, Map.MapCellY+Map.MapCellHeight);

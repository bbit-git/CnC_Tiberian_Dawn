...
void MapClass::Init_Cells(void)
{
	...
	for (int idx = 0; idx < MAP_CELL_TOTAL; idx++) {
		Map[idx].Overlay = OVERLAY_NONE;
		Map[idx].Smudge = SMUDGE_NONE;
		Map[idx].TType = TEMPLATE_NONE;
		Map[idx].Owner = HOUSE_NONE;
		Map[idx].InfType = HOUSE_NONE;
	}
#endif
}
	...
	for (i = 0; i < MAP_CELL_TOTAL; i++) {
		...
		*/
		if (temp.TType != TEMPLATE_CLEAR1 && temp.TType != TEMPLATE_NONE) {
			...
			if (shape) {
				rawmap = Get_Icon_Set_Map(const_cast<void*>(shape));
				if (rawmap) {
					map = (char*)rawmap;
					if (map[temp.TIcon] == -1) {
						temp.TIcon = 0;
...
void MapClass::Logic(void)
{
	...
	*/
	int subcount = 30;
	int idx;
	for (idx = TiberiumScan; idx < MAP_CELL_TOTAL; idx++) {
		CELL cell = idx;
		if (!IsForwardScan) cell = (MAP_CELL_TOTAL-1) - idx;
			...
			for (int i = 0; i < tries; i++) {
				...
				*/
				if (Map.In_Radar(cell)) {
					...
					for (FacingType idx = FACING_N; idx < FACING_COUNT; idx++) {
						CellClass *newcell = &(*this)[cell].Adjacent_Cell(idx+offset);
...
if (BlubCell->Overlapper[1]) {
	...
	for (cell = 0; cell < MAP_CELL_TOTAL; cell++) {
		...
		if (ttype != TEMPLATE_NONE) {
			...
			ticon = (*this)[cell].TIcon;
			Mem_Copy(Get_Icon_Set_Map(const_cast<void*>(tclass->Get_Image_Data())), map,
...
ObjectClass * MapClass::Close_Object(COORDINATE coord) const
{
	...
	*/
	...
	for (int idx = 0; idx < (sizeof(_offsets) / sizeof(_offsets[0])); idx++) {

		/*
		**	Examine the cell for close object. Make sure that the cell actually is a
		**	legal one.
		*/
		CELL newcell = cell + _offsets[idx];
		if (In_Radar(newcell)) {

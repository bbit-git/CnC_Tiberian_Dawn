	...
	if (name) {
		...
		*/
		...
		if ((vqa = VQA_Alloc()) != NULL) {
			VQA_Init(vqa, (void*)MixFileHandler);
...
void Dump_Heap_Pointers( void )
{
	...
	while ( ptr ) {
		...
		if ( ((intptr_t)cptr & 0xff000000) ||
			((intptr_t)dptr & 0xff000000) ||
			((intptr_t)lptr & 0xff000000) ) {
...
{
	/* Steam remastered data — no CD check needed */
	(void)cd;
	return true;
}

#if 0 /* Original CD checking code disabled */
bool Force_CD_Available_ORIGINAL(int cd)
{
#ifndef DEMO
	static int _last = -1;
	int open_failed;
...
//			}
			...
			*/
			...
			Set_Font(font);
			Mem_Copy(_hold, const_cast<void*>(Get_Font_Palette_Ptr()), 256);
			Set_Logic_Page(oldpage);
			InMainLoop = old_in_main_loop;
		}
	}
...
#endif /* Original CD checking code */

...
unsigned long Build_Frame(void const *dataptr, unsigned short framenumber, void *buffptr)
{
	...
	if (UseBigShapeBuffer){
		...
		*/
		...
		if ((Frame & 0xff) == 0){
			...
			if (show_info){
				...
				sprintf (crap, "C&C95 - %d Kb Used in big shape buffer.\n", (unsigned)((uintptr_t)BigShapeBufferPtr - (uintptr_t)BigShapeBufferStart)/1024);
				CCDebugString (crap);

				sprintf (crap, "C&C95 - %d Kb Used in theater shape buffer.\n", (unsigned)((uintptr_t)TheaterShapeBufferPtr - (uintptr_t)TheaterShapeBufferStart)/1024);
				CCDebugString (crap);
				show_info = false;
			}
		...
		*/
		if (( (uintptr_t)BigShapeBufferStart + BigShapeBufferLength) - (uintptr_t)BigShapeBufferPtr < 128*1024){

...
//	GradFont6Ptr = MixFileClass::Retrieve("GRAD6FNT.FNT");
	...
	Palette = new(MEM_CLEAR|MEM_REAL) unsigned char[768];
	...
	*/
	...
	/* Copy to Palette too — Load_Title_Screen would normally set this from PCX */
	memcpy(Palette, GamePalette, 768);
	memcpy(OriginalPalette, GamePalette, 768);
...
//	if (hCCLibrary) FreeLibrary(hCCLibrary);
	...
	*/
	...
	/* Retry loading language strings — they may be in GENERAL.MIX or CONQUER.MIX
...
bool Select_Game(bool fade)
{
	...
	*/
	...
	PlayerLoses = false;
	MPlayerObiWan = false;
	/* Debug_Unshroud = true; — removed, shroud enabled */
							...
							if ( NullModem.Num_Connections() ) {
								...
								if ((GameToPlay == GAME_NULL_MODEM &&
									...
									if ( !Com_Scenario_Dialog() ) {
										GameToPlay = (GameType)Select_Serial_Dialog();
